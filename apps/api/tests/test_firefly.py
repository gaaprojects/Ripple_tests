...
_FIXTURE_REFERENCE = "test"

# Precomputed: sha256("pay-test-001|5000.00|RLUSD|rDestination123|test")
_FIXTURE_DIGEST = hashlib.sha256(
    f"{_FIXTURE_ID}|{_FIXTURE_AMOUNT:.2f}|{_FIXTURE_CURRENCY}|{_FIXTURE_DEST}|{_FIXTURE_REFERENCE}".encode()
).hexdigest()


def test_canonical_payload_format():
    result = canonical_payload(
        _FIXTURE_ID, _FIXTURE_AMOUNT, _FIXTURE_CURRENCY, _FIXTURE_DEST, _FIXTURE_REFERENCE
    )
    assert result == "pay-test-001|5000.00|RLUSD|rDestination123|test"
...
def test_amount_formatting_int_float_equivalent():
    ...
    assert canonical_payload(_FIXTURE_ID, 5000, _FIXTURE_CURRENCY, _FIXTURE_DEST, _FIXTURE_REFERENCE) == \
           canonical_payload(_FIXTURE_ID, 5000.0, _FIXTURE_CURRENCY, _FIXTURE_DEST, _FIXTURE_REFERENCE)
...
def test_amount_formatting_rounds_half_even():
    ...
    result = canonical_payload(
        _FIXTURE_ID, 5000.005, _FIXTURE_CURRENCY, _FIXTURE_DEST, _FIXTURE_REFERENCE
    )
...
def test_reference_is_bound_into_digest():
    # WYSIWYS: the reference shown on the device must change the signed digest,
    # so altering it after signing breaks verification.
    base = challenge_digest("pay-001", 500.0, "RLUSD", "rAlice", "INV-1")
    tampered = challenge_digest("pay-001", 500.0, "RLUSD", "rAlice", "INV-2")
...
def test_different_payments_produce_different_digests():
    d1 = challenge_digest("pay-001", 500.0, "RLUSD", "rAlice", "ref")
    d2 = challenge_digest("pay-002", 500.0, "RLUSD", "rAlice", "ref")
    d3 = challenge_digest("pay-001", 50000.0, "RLUSD", "rAlice", "ref")
    d4 = challenge_digest("pay-001", 500.0, "RLUSD", "rBob", "ref")
    d5 = challenge_digest("pay-001", 500.0, "RLUSD", "rAlice", "other-ref")
