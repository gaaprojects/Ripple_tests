...
async def _persist_payment(payment: Payment) -> None:
    ...
    try:
        async with db.session_factory() as session:
            row = _payment_to_row(payment, existing=None)
...
async def _persist_credential(record: CredentialRecord) -> None:
    ...
    try:
        async with db.session_factory() as session:
            row = _credential_to_row(record, existing=None)
            await session.merge(row)
            await session.commit()
    except Exception as exc:
...
def _payment_to_row(payment: Payment, *, existing=None) -> PaymentRecord:
    row = PaymentRecord(
        id=payment.id,
...
def _row_to_payment(row: PaymentRecord) -> Payment:
    ...
    return Payment(
        ...
        compliance=_deserialize_compliance(compliance_data) if compliance_data else None,
        policy_decision=PolicyDecision(**_camel_to_snake(policy_data)) if policy_data else None,
...
def _credential_to_row(record: CredentialRecord, *, existing=None) -> CredentialRecordDB:
...
def _deserialize_compliance(data: dict) -> ComplianceResult:
    matches = [
        SanctionsMatch(
            id=m["id"],
            caption=m["caption"],
            schema_=m.get("schema", ""),
    ...
    public = data.get("publicIntel")
    return ComplianceResult(
        aml_score=data["amlScore"],
        sanctioned=data["sanctioned"],
        flags=data.get("flags", []),
        explanation=data.get("explanation", ""),
        sanctions_matches=matches,
        public_intel=PublicIntelResult(**_camel_to_snake(public)) if public else None,
        credential=CredentialStatus(**_camel_to_snake(data["credential"])) if data.get("credential") else None,
    )


# Minimal camelCase→snake_case for the small set of fields we need to deserialise.
_CAMEL_MAP = {
    ...
    "sanctionsMatches": "sanctions_matches",
    "credentialType": "credential_type",
}


def _camel_to_snake(data: dict) -> dict:
    return {_CAMEL_MAP.get(k, k): v for k, v in data.items()}
