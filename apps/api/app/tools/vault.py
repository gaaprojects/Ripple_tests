...
async def create_vault(asset_currency: str, asset_issuer: str) -> VaultCreateResult:
    ...
    if settings.use_mock_xrpl:
        ...
        tx_hash = _mock_hash("vault_create", vault_id)
    ...
    async with xrpl_client.async_client(endpoint=settings.vault_xrpl_endpoint) as client:
...
async def deposit(vault_id: str, amount: float) -> VaultOpResult:
    ...
    if settings.use_mock_xrpl:
        ...
        tx_hash = _mock_hash("vault_deposit", f"{vault_id}:{actual}")
...
async def withdraw(vault_id: str, amount: float) -> VaultOpResult:
    ...
    if settings.use_mock_xrpl:
        ...
        shares = actual / 100.0
        tx_hash = _mock_hash("vault_withdraw", f"{vault_id}:{actual}")
